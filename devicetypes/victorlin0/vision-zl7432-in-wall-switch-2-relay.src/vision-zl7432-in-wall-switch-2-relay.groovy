/**
 *  
 *  Vision ZL7432 In Wall Switch, 2 relay
 *
 *  Author: Victor Lin
 *
 *  Todo: Need to test out figreprint to see if that works.
 *  
 *
 ...
 *
 *  Special thanks to Justin Ellison and Eric Maycock for doing the bulk of the work with the Philio PAN04 module, without his work
...
metadata {
    definition (name: "Vision ZL7432 In wall switch 2 relay", namespace: "victorlin0", author: "Victor Lin") {
        capability "Polling"
        capability "Refresh"
        capability "Switch"
...
def parse(String description) {
    ...
    if (cmd) {
        ...
        log.debug "--> ${cmd} to ${result.inspect()}"
    } else {
...
def zwaveEvent(physicalgraph.zwave.commands.basicv1.BasicReport cmd) {
   log.debug "basicv1.BasicReport $cmd"
    def result
    if (cmd.value == 0) {
        result = createEvent(name: "switch1", value: "off")
    } else {
        result = createEvent(name: "switch1", value: "on")
    }
    return result
}

def zwaveEvent(physicalgraph.zwave.commands.switchbinaryv1.SwitchBinaryReport cmd) {
   log.debug "switchbinaryv1.SwitchBinaryReport $cmd"
 
    sendEvent(name: "switch1", value: cmd.value ? "on" : "off", type: "digital")
    def result = []
...
def zwaveEvent(physicalgraph.zwave.commands.multichannelv3.MultiChannelCmdEncap cmd) {
    ...
    log.debug "multichannelv3.MultiChannelMultiChannelCmdEncap $cmd"
...
def zwaveEvent(physicalgraph.zwave.Command cmd) {
    ...
    log.debug "Command not handled $cmd"
